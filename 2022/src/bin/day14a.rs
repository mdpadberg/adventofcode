...
fn solve(input: String) -> usize {
    ...
    let endless_void = grid.iter().map(|(key, _)| key.y).max().unwrap() + 1;
    loop {
        let mut sand = Point { x: 500, y: 0 };
        loop {
            let is_falling = sand.fall(&grid);
            if is_falling && sand.y < endless_void {
                continue;
            } else {
                break;
            }
        }
        if sand.y >= endless_void {
            break;
        }
...
mod test {
    ...
    #[test]
    fn solvetest() {
        assert_eq!(24, solve(read_test_data_for_day("14-0").unwrap()));
    }
}
