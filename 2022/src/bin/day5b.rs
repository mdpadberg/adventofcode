...
fn solve(input: String) -> String {
    ...
    let mut current_crate_situation: Vec<Vec<char>> = input
        ...
        .collect();
    for m in moves {
        let len: &usize = &current_crate_situation[m.from - 1].len();
        let mut crates_that_needs_to_be_moved: Vec<char> = current_crate_situation[m.from - 1].drain(*len-m.amount..).collect();
        current_crate_situation[m.to-1].append(&mut crates_that_needs_to_be_moved);
    }
    current_crate_situation
        .into_iter()
...
mod test {
    ...
    fn solvetest() {
        assert_eq!(
            String::from("MCD"),
            solve(read_test_data_for_day("5-0").unwrap())
        );
    }
}
