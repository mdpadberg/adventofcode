...
fn solve(input: String) -> usize {
    ...
    for direction in directions {
        ...
        tail.follow(&head);
        tail_visited.insert(tail.position);
    }
    tail_visited.len()
}
...
impl Thing {
    fn move_to(&mut self, direction: &Direction) {
        self.position = self.position.move_to(direction);
    }
    ...
    //  h . . . i
    //    j k l
    fn follow(&mut self, thing: &Thing) {
        match (
            thing.position.y - self.position.y,
            thing.position.x - self.position.x,
        ) {
            //a
            (2, -1) => self.position = self.position.move_to(&Direction::UP_AND_LEFT),
            //b
            (2, 0) => self.position = self.position.move_to(&Direction::UP),
            //c
            (2, 1) => self.position = self.position.move_to(&Direction::UP_AND_RIGHT),
            //d
            (1, -2) => self.position = self.position.move_to(&Direction::UP_AND_LEFT),
            //e
            (1, 2) => self.position = self.position.move_to(&Direction::UP_AND_RIGHT),
            //f
            (0, -2) => self.position = self.position.move_to(&Direction::LEFT),
            //g
            (0, 2) => self.position = self.position.move_to(&Direction::RIGHT),
            //h
            (-1, -2) => self.position = self.position.move_to(&Direction::DOWN_AND_LEFT),
            //i
            (-1, 2) => self.position = self.position.move_to(&Direction::DOWN_AND_RIGHT),
            //j
            (-2, -1) => self.position = self.position.move_to(&Direction::DOWN_AND_LEFT),
            //k
            (-2, 0) => self.position = self.position.move_to(&Direction::DOWN),
            //l
            (-2, 1) => self.position = self.position.move_to(&Direction::DOWN_AND_RIGHT),
            //dont move
            _ => {},
        }
    }
...
mod test {
    ...
    #[test]
    fn solvetest() {
        assert_eq!(13, solve(read_test_data_for_day("9-3").unwrap()));
    }
}
