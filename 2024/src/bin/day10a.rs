...
impl Trail {
    fn first_and_last(&self) -> Option<(Coordinates, Coordinates)> {
...
fn solve(input: String) -> usize {
    ...
    valid_trails
        .into_iter()
        .filter_map(|trail| trail.first_and_last())
        .collect::<HashSet<(Coordinates, Coordinates)>>()
        .len()
}
...
mod test {
    ...
    #[test]
    fn solvetest() {
        assert_eq!(2, solve(read_test_data_for_day("10-1").unwrap()));
        assert_eq!(4, solve(read_test_data_for_day("10-2").unwrap()));
        assert_eq!(3, solve(read_test_data_for_day("10-3").unwrap()));
        assert_eq!(36, solve(read_test_data_for_day("10-4").unwrap()));
    }
}
